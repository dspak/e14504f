...
# Write nexus and phy files for each locus
...
mrbayes(interleaved, "bayes/all.nex", nst = 6, rates = "invgamma")

interleaved_rax <- interleaved
rownames(interleaved_rax) <- gsub(" ", "_", rownames(interleaved_rax))
rownames(interleaved_rax) <- gsub(":|,|;|\\(|\\)", "_", rownames(interleaved_rax))

# Create phylip output for raxml
write.phy(interleaved_rax, "raxml/all.phy")

# Write Mr Bayes submission file to move to Grace and run there.
fileConn <- file("bayes/mrbayes_e14504f_all.sh")
...
system({
  "raxmlHPC-AVX -s raxml/all.phy -m GTRGAMMA -n all_1 -p 12345 -x 12345 -# 100"
})
system({
  "raxmlHPC-AVX -m GTRGAMMA -J MRE -z RAxML_bootstrap.all_1 -n tre"
})
?pml
```
