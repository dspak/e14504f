...
```{r merge Nneomas e14504f fastas with genbank fastas from table}
bfastas <- list()
...
for (i in names(ffastas)) {
  # Select the matching ggfastas list component as n
  n <- names(ggfastas)[grep(i, names(ggfastas))]
  # Iterate over all sequences in n
  for (j in 1:length(ggfastas[[n]])) {
    x <- ffastas[[i]][[1]]
    y <- ggfastas[[n]][j][[1]]
    ...
    tmp <- pairwiseAlignment(x, y, type = "local", substitutionMatrix = sigma,
